...
export class StrategyEvolver {
  ...
  mutate(dna: StrategyDNA): StrategyDNA {
    ...
    const changes = keysToMutate.map(k => `${k}: ${dna.params[k]} → ${mutated.params[k]}`).join('  ');
    console.log(`[MUTATE] ${mutated.name}  ${changes}  (id: ${mutated.id.slice(0, 8)}, parent: ${mutated.parentId?.slice(0, 8)})`);
